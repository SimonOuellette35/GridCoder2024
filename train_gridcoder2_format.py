...
# This script loads training and validation files in JSON format (GridCoder 2 format) 
    ...
    try:
        with open(filename, 'r') as f:
            ...
            for data in data_list:
                # 0: input grid as token sequence
                # 1: output grid as token sequence
                # 2: program in label sequence format
                row = []

                tmp_grid_sequence = data['input_sequence']
                
                input_grid = tmp_grid_sequence[:931]
                output_grid = tmp_grid_sequence[931:]
                
                row.append(input_grid)
                row.append(output_grid)

                label_seq = convert_to_label_seq(data['prog'])
                row.append(label_seq)
...
def train():
    ...
    validation_data = load_data('validation.json')
    ...
    for row in training_data:
        ...
        if len(batch) == batch_size:
            if batch_num >= RESUME:
                ...
                val_batch = random.sample(validation_data, min(1000, len(validation_data)))
